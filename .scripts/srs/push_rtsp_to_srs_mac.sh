#!/bin/bash
# Mac环境下使用ffmpeg将RTSP流推送到SRS服务器
# 使用方法: ./push_rtsp_to_srs_mac.sh -u "rtsp://192.168.1.100:554/stream" -h "192.168.1.200" -p 1935 -a "live" -s "test"
...
detect_ffmpeg() {
    ...
    local common_paths=(
        ...
        "/usr/bin/ffmpeg"                # 系统路径
        "$HOME/homebrew/bin/ffmpeg"      # 用户安装的Homebrew
    )
    
    for path in "${common_paths[@]}"; do
        if [[ -f "$path" && -x "$path" ]]; then
            FFMPEG_PATH="$path"
            echo -e "${GREEN}自动检测到ffmpeg: $path${NC}"
            return
        fi
    done
}
...
if ! command -v "$FFMPEG_PATH" &> /dev/null; then
    ...
    echo -e "${YELLOW}或者使用 -f 参数指定ffmpeg的完整路径，例如: -f '/opt/homebrew/bin/ffmpeg'${NC}"
    echo ""
    echo "在Mac上安装ffmpeg:"
    echo "  1. 使用Homebrew安装（推荐）:"
    echo "     brew install ffmpeg"
    echo ""
    echo "  2. 如果使用Homebrew但找不到，尝试:"
    echo "     export PATH=\"/opt/homebrew/bin:\$PATH\"  # Apple Silicon Mac"
    echo "     或"
