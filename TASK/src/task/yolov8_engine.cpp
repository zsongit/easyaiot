...
int Yolov8Engine::LoadModel(const char *model_path)
{
...
int Yolov8Engine::Preprocess(const cv::Mat &img, const std::string process_type, cv::Mat &image_letterbox)
{
...
int Yolov8Engine::Postprocess(const cv::Mat &img, std::vector<Detection> &objects)
{
    void *output_data[6];
    for (int i = 0; i < 6; i++)
    {
...
int Yolov8Engine::Run(const cv::Mat &img, std::vector<Detection> &objects)
{
