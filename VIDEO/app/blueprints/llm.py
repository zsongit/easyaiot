...
def test_aliyun_qwenvl3(model: LLMModel) -> dict:
    ...
    try:
        ...
        # 构建API端点
        if model.base_url.endswith('/'):
            api_url = f"{model.base_url}v1/chat/completions"
        else:
            api_url = f"{model.base_url}/v1/chat/completions"
...
def call_aliyun_vision_with_mode(model: LLMModel, base64_image: str, prompt: str, mode: str) -> dict:
    ...
    if mode == 'inference':
        # 视觉推理：更注重对象识别和场景分析
        enhanced_prompt = f"作为视觉推理专家，请分析这张图片：{prompt}"
    elif mode == 'understanding':
        # 视觉理解：更注重深度理解和语义分析
        enhanced_prompt = f"作为视觉理解专家，请深入理解这张图片：{prompt}"
    elif mode == 'deep-thinking':
        # 深度思考：更注重多角度分析和推理
        enhanced_prompt = f"作为深度思考专家，请对这张图片进行多角度深度分析：{prompt}"
    else:
